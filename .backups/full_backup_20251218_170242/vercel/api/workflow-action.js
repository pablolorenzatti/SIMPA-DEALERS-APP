...
for (const configPath of configPaths) {
  try {
    if (fs.existsSync(configPath)) {
      ...
      console.log(`[workflow-action] config.js cargado desde: ${configPath}`);
      break;
    }
  } catch (error) {
...
if (!configUtils) {
  try {
    ...
    console.log('[workflow-action] config.js cargado con require directo');
  } catch (error) {
    console.warn('[workflow-action] No se pudo cargar config.js, usando fallback básico');
    configUtils = null;
  }
}
...
for (const razonesSocialesPath of razonesSocialesPaths) {
  try {
    if (fs.existsSync(razonesSocialesPath)) {
      ...
      console.log(`[workflow-action] ✅ razones-sociales.json cargado`);
      break;
    }
  } catch (error) {
    // Continuar con el siguiente path
...
if (!razonesSocialesConfig) {
  try {
    ...
    console.log('[workflow-action] ✅ razones-sociales.json cargado con require');
  } catch (error) {
    try {
      razonesSocialesConfig = require('../src/config/razones-sociales.json');
      console.log('[workflow-action] ✅ razones-sociales.json cargado con require relativo');
    } catch (error2) {
      console.error('[workflow-action] ❌ CRITICAL: No se pudo cargar razones-sociales.json. La acción fallará si se requieren tokens.');
      // No fallback
    }
  }
}

// Cargar configuración de propiedades desde properties-config.json
...
for (const propertiesConfigPath of propertiesConfigPaths) {
  try {
    if (fs.existsSync(propertiesConfigPath)) {
      ...
      console.log(`[workflow-action] ✅ properties-config.json cargado desde: ${propertiesConfigPath}`);
      break;
    }
  } catch (error) {
    // Continuar con el siguiente path
...
if (!propertiesConfig) {
  try {
    ...
    console.log('[workflow-action] ✅ properties-config.json cargado con require directo');
  } catch (error) {
    try {
      propertiesConfig = require('../src/config/properties-config.json');
      console.log('[workflow-action] ✅ properties-config.json cargado con require relativo');
    } catch (error2) {
      console.error('[workflow-action] ❌ CRITICAL: No se pudo cargar properties-config.json.');
      // No fallback
    }
  }
}

// Cargar configuración de modelos por marca
let modelsByBrandConfig;
const modelsByBrandPaths = [
  path.join(__dirname, '../../src/config/models-by-brand.json'),
  path.join(__dirname, '../src/config/models-by-brand.json'),
  path.join(__dirname, 'src/config/models-by-brand.json'),
  path.join(process.cwd(), 'src/config/models-by-brand.json'),
  '/var/task/src/config/models-by-brand.json',
  '/var/task/vercel/src/config/models-by-brand.json',
  '/var/task/api/src/config/models-by-brand.json'
];

for (const modelsByBrandPath of modelsByBrandPaths) {
  try {
    if (fs.existsSync(modelsByBrandPath)) {
      modelsByBrandConfig = JSON.parse(fs.readFileSync(modelsByBrandPath, 'utf8'));
      console.log(`[workflow-action] ✅ models-by-brand.json cargado`);
      break;
    }
  } catch (error) {
    // Continuar con el siguiente path
...
if (!modelsByBrandConfig) {
  try {
    modelsByBrandConfig = require('../../src/config/models-by-brand.json');
    console.log('[workflow-action] ✅ models-by-brand.json cargado con require');
  } catch (error) {
    try {
      modelsByBrandConfig = require('../src/config/models-by-brand.json');
      console.log('[workflow-action] ✅ models-by-brand.json cargado con require relativo');
    } catch (error2) {
      console.error('[workflow-action] ❌ CRITICAL: No se pudo cargar models-by-brand.json.');
      // No fallback
    }
  }
}

// Inicializar PropertyService
...
async function procesarPropiedadesModelosBulk(marca, modelo, stats) {
  ...
  for (const razonSocial of razonesSocialesConMarca) {
    ...
    try {
      ...
      await procesarPropiedadesModelos(hubspotClient, context, tempStats);
...
async function procesarPropiedadesModelos(hubspotClient, context, stats) {
  ...
  try {
    ...
    switch (tipoProceso) {
      case 'propiedades_concesionarios':
        ...
        await procesarPropiedadesConcesionarios(hubspotClient, context, stats);
        break;
      ...
      case 'propiedades_modelos':
        ...
        if (!razonSocial) {
          ...
          await procesarPropiedadesModelosBulk(marca, modelo, stats);
          ...
          await procesarPropiedadesModelos(hubspotClient, context, stats);
        }
        break;
...
async function procesarPropiedadesConcesionarios(hubspotClient, context, stats) {
